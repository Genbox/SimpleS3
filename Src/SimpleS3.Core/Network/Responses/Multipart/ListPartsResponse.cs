...
public class ListPartsResponse : BaseResponse, IHasRequestCharged, IHasAbort, IHasTruncated, IHasStorageClass, IHasUploadId, IHasBucketName, IHasObjectKey
{
    ...
    /// <summary>When a list is truncated, this element specifies the last part in the list, as well as the value to use for
    /// the part-number-marker request parameter in a subsequent request.</summary>
    ...
    /// <summary>Identifies the object owner, after the object is created. If multipart upload is initiated by an IAM user,
    /// this element provides the parent account ID and display name.</summary>
