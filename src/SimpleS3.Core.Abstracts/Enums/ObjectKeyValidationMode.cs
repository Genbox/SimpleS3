...
public enum ObjectKeyValidationMode
{
    ...
    ///<summary>Provider specific validation will be performed in strict mode when using a provider. Strict mode checks that only safe and backwards compatible characters are used. When not using a provider, it will default to <see cref="Unrestricted"/></summary>
    DefaultStrict,
