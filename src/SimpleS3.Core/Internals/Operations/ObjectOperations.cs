...
internal class ObjectOperations : IObjectOperations
{
    ...
    public Task<ListObjectVersionsResponse> ListObjectVersionsAsync(ListObjectVersionsRequest request, CancellationToken token = default)
    {
        return _requestHandler.SendRequestAsync<ListObjectVersionsRequest, ListObjectVersionsResponse>(request, token);
    }
