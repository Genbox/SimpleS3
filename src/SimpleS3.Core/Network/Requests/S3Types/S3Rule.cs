...
public class S3Rule
{
    ...
    /// <summary>Specifies the days since the initiation of an incomplete multipart upload that Amazon S3 will wait before
    /// permanently removing all parts of the upload.</summary>
    public int? AbortIncompleteMultipartUploadDays { get; set; }

    /// <summary>Specifies the expiration for the lifecycle of the object in the form of date, days and, whether the object has
    /// a delete marker.</summary>
    public S3Expiration? Expiration { get; set; }

    /// <summary>The Filter is used to identify objects that a Lifecycle Rule applies to. A Filter must have exactly one of
    /// Prefix, Tag, or And specified.</summary>
    ...
    /// <summary>Specifies the transition rule for the lifecycle rule that describes when noncurrent objects transition to a
    /// specific storage class. If your bucket is versioning-enabled (or versioning is suspended), you can set this action to
    /// request that Amazon S3 transition noncurrent object versions to a specific storage class at a set period in the
    /// object's lifetime.</summary>
